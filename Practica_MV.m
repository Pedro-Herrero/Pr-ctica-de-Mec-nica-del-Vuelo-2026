...
for i = 1:length(R_list)
    plot(phi_deg, n_matrix(i,:), 'LineWidth', 2, ...
         'DisplayName', ['R = ' num2str(R_list(i)) ' m']);
end

% Referencias de límites
yline(n_max, 'r--', 'LineWidth', 1.5, 'HandleVisibility','off');
yline(n_min, 'r:',  'LineWidth', 1.5, 'HandleVisibility','off');
yline(0, 'k-', 'HandleVisibility','off');
grid on; legend('Location','northeastoutside');
xlabel('\phi [°]'); ylabel('n [g]');
title(['Efecto del Radio en el Factor de Carga (V = ' num2str(V) ' m/s)']);
xlim([0 180]); xticks(0:30:180);
...
% Referencias de límites
yline(CLmax, 'r--', 'LineWidth', 1.5, 'HandleVisibility','off');
yline(CLmin, 'r:',  'LineWidth', 1.5, 'HandleVisibility','off');
yline(0, 'k-', 'HandleVisibility','off');
grid on; legend('Location','northeastoutside');
xlabel('\phi [°]'); ylabel('C_L [-]');
title(['Efecto del Radio en el C_L Requerido (V = ' num2str(V) ' m/s)']);
xlim([0 180]); xticks(0:30:180);
